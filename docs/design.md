...
**持续性标记**：写入需要显式的持续表达（"以后/默认/一直…"），对当前这条回答的要求不落库。理由与判据见 9.2。
...
| `history` | 被覆盖的旧值修订链（保留最近 5 版），也是权威性高水位的读取来源 |

作用域不是 `MemoryEntry` 的字段，而是编码在**存储键**里（`__thread_<threadID>__<key>`）——见 9.2。

### 9.2 写入与冲突消解

`UpsertPreference` 为唯一写入口：key 已存在且值不同 → 旧值进 `history`（新值生效）；值相同 → no-op。

**作用域（scope）决定条目写到哪个命名空间**：

| 作用域 | 存储键 | 可见范围 | 谁产生 |
|--------|--------|----------|--------|
| `ScopeUser`（默认） | `key` | 该用户所有会话 | 持续标记的表述、身份事实、API 手写、整合产物 |
| `ScopeThread` | `__thread_<threadID>__<key>` | 仅该会话 | 无持续标记的表现类请求 |

作用域存在的理由：**存储模型没有别的办法表达"只在这个会话里"**。条目一旦写进用户命名空间就是全局可见，所以"请详细说明这个函数"此前只能二选一——要么全局记录（错），要么丢弃。有了会话级作用域，它被记在**提问的那个会话里**，别处不受影响。

会话级条目复用保留键，因此自动继承用户隔离与后端切换（memory / file / postgres），零新增存储代码；既有的保留键过滤器也让它们不出现在记忆列表、检索打分、整合与记忆写入 API 中。

**两条清理路径**，缺一不可——没人清理的会话级条目会变成不可达却仍算作用户数据的残留：
...
- **值仍然最后写入者胜出**——后来的表述确实可能是改变主意，一刀切禁止覆盖会让"我改用 Rust 了"这类真实变化永远写不进去。
- **`importance` 只能被不低于"历史最强证据"的写入修改**。`entryAuthority` 取当前 `Source` 与 `History` 链中各来源的最大值。若不设此守卫，一次默认重要性的 LLM 提取就能把手写偏好踢出核心档——而核心档正是保证它被注入的唯一机制（见 9.3）。
- **`Source` 如实记录当前值的写入者**，不做保留。让标签粘住高权威会让它不再表示"这个值是谁写的"，反而误导；"用户曾明说过"这一事实由 `History` 链承载，权威性也从那里读。

> 高水位必须读 `History` 而不是只看当前 `Source`：后者只能挡住第一次弱写入——第一次写入后 `Source` 已变成 `llm_extracted`，第二次弱写入与之同级就畅通了。实测（注入验证）朴素比较恰好在第二次写入时失守。

**提取策略为"LLM 主导 + 规则兜底"**：配置了模型时，LLM 提取拥有本轮决定权（结果经 `memoryScopeFor` 判定作用域与是否准入）；规则提取仅在无模型或模型调用/解析失败时兜底（如 mock 模式），保证无真实 LLM 时提取依然确定可用。带偏好信号的消息额外写入 `type=episode` 情景条目并进入向量索引；普通闲聊不产生任何写入。

**持续性标记（standing marker）决定作用域**，两条路径共用同一判据：

| 类别 | 例子 | 结果 |
|------|------|------|
| 表现类 + 持续标记 | 以后请用简洁的方式回答 | 用户级 `answer_style` |
| 表现类 + 无标记 | 请详细说明这个函数 / 这个回答太长了，请简短一些 / 简洁点 / 展开说说 | **会话级** `answer_style` |
| 身份类（语言/框架/编辑器/城市）+ 无标记 | 用Python写这个 | 不写入（没有合理的会话级读法） |
| 身份类 + 标记 | 以后都用Go | 用户级 |
| 闭集键值未逐字出现 | 消息没提 Python 但模型"提取"了 Python | 不写入（防幻觉） |

`memoryScopeFor` 是回答"作用域与准入"的唯一位置，因为这两个问题其实是同一个：**这句话管多远**。

> 门槛的由来：此前实测四个日常的一次性请求全部写成了**永久全局**偏好，一句话就让该用户以后所有会话都变风格，来回说还会让画像随单轮措辞震荡。收窄词汇表是让提取器开始改写用户画像的常见方式，因此标记表要窄——但把请求**丢掉**又浪费了它，作用域正是为此而加。
...
**核心偏好有保留席位，不参与竞争**：`preference` / `identity` / `rule` 且 `importance ≥ 4` 的条目先无条件放入预算，剩余预算才轮到"向量召回 + 其余条目按分数竞争"。

理由是不对称的分数上限：偏好的上限是 `importance/5 + 近因` ≈ 1.6（默认重要性 3），而命中查询的情景可达 `0.4×0.8 + 2.0 = 2.32`。实测 8 条命中查询的情景把 `preferred_language` **完全挤出**注入内容——用户问"帮我写部署脚本"，助手连他用什么语言都不知道。重要性是"无论问什么这条都重要"的显式声明，因此这类条目不该参与竞争。`PutPreference`（用户经 API 手写的记忆）直接按核心重要性写入：手写本身就是重要性声明，而 API 没有 importance 字段。

> 一个容易踩的坑：`FormatVectorResultsWithin` 的 `maxTokens=0` 表示**不限**而非"没有预算"。核心偏好吃满预算时必须**跳过**向量召回，传 0 会让预算彻底失效。

**关键词重叠必须对中文可用**：打分用的分词是 `overlapTokens`，与 `tokenize` **刻意不同**。`tokenize` 把连续中文串当作一个 token，于是中文查询只能与条目中完全相同的片段匹配——实测自然中文查询的重叠度为 **0**，查询感知通道对本项目主要语言完全失效。`overlapTokens` 对 CJK 串输出字符的一元组与二元组，于是"帮我写个部署脚本"与"部署脚本要用 Go 写"共享 `部署`/`署脚`/`脚本`。

之所以不直接改 `tokenize`：它喂给 `hashEmbed`，而 hash 模式的 `VECTOR_MIN_SCORE` 是**针对当前分词下的分数分布实测标定的**——换分词会移动整个分布，阈值必须重新实测。检索打分另用一份分词，正是为了让 `tokenize` 保持原样（有测试钉住它"整串中文仍是一个 token"）。

> 更正一处早前的说法：这里曾经写成"改了会让已存的 hash 向量失效"，那是错的。项目里**没有任何持久化向量库**——`InMemoryVectorStore` 与 `chromem.NewDB()` 都是进程内的，向量是派生状态，`ensureVectorIndex` 每次启动从 KV 文本懒重建。所以改 `hashEmbed` 的真实代价只是**重新标定阈值**，不需要迁移；只是这件事必须先做，否则阈值会悄悄失配（这正是 `VECTOR_MIN_SCORE` 那个"尺度必须匹配"教训的同一个坑）。

**预算分配按作用域由具体到宽泛**：会话级偏好 → 核心偏好 → 向量召回（在剩余预算内）→ 其余条目按分数竞争 → 文档（独立预算），因此注入总量不会超出预算。

会话级条目排在最前且不参与打分：它们是为**这个会话**说的，相关性不是它们的准入条件，而数量只可能是寥寥几个。它们渲染在独立的小节里（`本次会话的偏好：`）而不是混进"用户记忆（确定性）"——模型不该把为一次对话提出的要求读成关于用户的事实。
...
文档片段是第三个注入来源，与记忆并列但**完全独立**：

| 维度 | 记忆检索 | 文档检索 |
|------|----------|----------|
| 回答的问题 | "我对这个用户了解什么" | "这份材料里写了什么" |
| 向量库 | 记忆索引（情景 episode） | **独立**的文档索引 |
| 预算 | `MEMORY_BUDGET_TOKENS`（默认 400） | `DOCUMENT_BUDGET_TOKENS`（默认 800） |
| 输出段 | `用户记忆（确定性）` / `用户历史相关记忆` / `相关历史记忆` | `【文档片段】`，每条 `[n] 文档名 · 片段 k：内容` |

**为什么用独立的向量库而不是加元数据过滤**：现有 `VectorStore` 接口没有集合/过滤参数，而 chromem 的 `where` 只支持精确等值匹配。若把文档分块与记忆 episode 放进同一个 per-user 集合，两者会在同一个 top-K 里按相似度混排——文档片段会被情景记忆挤掉，反之亦然。独立实例用结构性隔离解决了这个问题，不需要改接口语义。

**引用**：每条片段带序号，系统提示词要求模型使用文档内容时以 `[n]` 标注来源。

**重启后的懒重建**：两个向量后端都是进程内的（`chromem.NewDB()` 不落盘），所以重启后 KV 里的分块还在、向量索引没了。`ensureVectorIndex` 在用户首次检索时从 KV 读回分块与 episode 重新入库，进程内按用户标记一次。多实例各自重建，正确但有重复 embedding 成本。同一机制顺带修复了情景记忆的同类问题——此前重启后 episode 的向量那半条召回是静默失效的。

**相关度阈值必须匹配 embedding 的打分尺度**：真实 embedding 把相关结果放在 0.6–0.9，hash 伪嵌入则低一个量级。因此 `VECTOR_MIN_SCORE` 支持显式配置，未设置时按提供方自动取值（hash → 0.02，真实 embedding → 0.3）。

**hash 模式的维度必须远大于单个片段的特征数**（`hashEmbeddingDim = 8192`）。哈希特征一旦碰撞就无法区分，而一个 125 字符的中文片段贡献约 140 个特征（每个词一个、每对相邻字符一个）：在最初的 128 维里占用率 **73%**，碰撞主导了相似度。实测九组配对（含中英文、长短文本），128 维下**无关片段反超相关片段**（最差 −0.28），2048 维仍有一组倒挂（−0.060），4096 维一组（−0.025），**8192 维全部无倒挂**（16384 维无进一步改善）。倒挂比噪声更糟——它会主动误导检索，而不只是不精确。代价是每条索引 32KB 与 O(dim) 的余弦计算，对进程内索引可以接受。

**阈值是噪声下限，不是相关性判据**。8192 维下相关中文片段落在 0.043–0.19，无关中文片段实测 **0.000**，所以 0.02 能保留全部相关召回并丢掉纯碰撞噪声。英文无关片段仍有约 0.11（任意两段英文都共用虚词），此时靠**排序**区分而不是阈值——排序已由上面的配对断言守住。早前记录的 0.1 是针对 128 维那个更嘈杂的分布标定的，它会连同噪声一起丢掉相关的中文召回（0.043）。
