...
之所以不直接改 `tokenize`：它喂给 `hashEmbed`，而 hash 模式的 `VECTOR_MIN_SCORE`（0.1）是**针对当前分词下的分数分布实测标定的**——换分词会移动整个分布，阈值必须重新实测。检索打分另用一份分词，正是为了让 `tokenize` 保持原样（有测试钉住它"整串中文仍是一个 token"）。
...
**相关度阈值必须匹配 embedding 的打分尺度**：真实 embedding 把相关结果放在 0.6–0.9，而 hash 伪嵌入压缩在 0.15–0.4。用同一个 0.3 会让 hash 模式下的中文检索完全失效——实测正确的中文文档只有 0.168，而一个无关文档反而更高。因此 `VECTOR_MIN_SCORE` 支持显式配置，未设置时按提供方自动取值（hash → 0.1，真实 embedding → 0.3）。
