...
**持续性标记（standing marker）是写入的硬门槛**，两条路径都要求：

| 类别 | 例子 | 是否落库 |
|------|------|----------|
| 带持续标记 | 以后请用简洁的方式回答 / 以后说话随意一点 / 默认用 Go | ✅ |
| 无持续标记 | 请详细说明这个函数 / 这个回答太长了，请简短一些 / 简洁点 / 请用简洁的方式回答 | ❌ |

之所以划在这里：**数据模型没有会话级作用域**，条目一旦写入就是"该用户的全局偏好"。因此只有明确要求全局的表述才记录——"请用简洁的方式回答"和"请简洁回答"意思相同，都是对**当前这条回答**的要求，都不落库。防幻觉校验的三类判断由此变成：枚举类 key 要求值逐字出现；表现类 key（`answer_style` / `output_format` / `communication_style`）要求消息带持续标记；其余 key 放行（其值本就是转述）。

> 这个门槛是后加的，原因是实测发现四个日常的一次性请求全部写成了**永久全局**偏好：一句话就让该用户以后所有会话都变成该风格，而且来回说（"简洁点"→"详细讲讲"）会让画像随单轮措辞震荡。收窄词汇表是让提取器开始改写用户画像的常见方式，因此标记表要窄。
...
**预算分配**：核心偏好 → 向量召回（在剩余预算内）→ 其余条目按分数竞争，因此注入总量不会超出预算。
