...
| # | 限制 | 涉及模块 | 说明 |
|---|------|----------|------|
| 1 | 多用户隔离非自动强制 | 模块 01 | 隔离依赖下游 handler 主动使用 `AuthContext.UserID`，auth 层不自动强制 |
| 2 | 节点级中断需意图触发 | 模块 02 | 非默认启用，用户消息需包含"确认后再执行"等关键词 |
| 3 | HITL 为异步审批模式 | 模块 02 | 中断后 run 结束，通过独立 API 调用恢复，非"挂起等待"语义 |
| 4 | grep 使用硬编码 mock 数据 | 模块 03 | 搜索日志是示例场景，数据完全合成 |
| 5 | 密码使用无盐 SHA-256 | 模块 01 | 演示项目可接受的安全简化 |
