...
| 1.2 | 每次调用均校验 sessionId | ✅ | `auth.AuthMiddleware()` 对所有非登录路由校验；凭证可经 `Authorization: Bearer` 头（优先）或 `HttpOnly` 会话 Cookie 传递，不接受 URL 查询参数 |
...
| 1.11 | 会话凭证的浏览器安全 | ✅ | 登录下发 `HttpOnly` + `SameSite=Strict` 会话 Cookie：页面脚本读不到凭证（XSS 无法窃取），刷新页面保持登录；登出使 Cookie 立即过期；`Secure` 由 `SESSION_COOKIE_SECURE` 控制 |
...
| 3.3 | Supervisor 中心路由至 2~3 个子 Agent | ✅ | 3 个子 Agent：math / search / general；配置了 MCP 服务器时追加第 4 个 `mcp_agent` 承载远端工具（未发现工具则不创建） |
| 3.4 | 外部工具接入（MCP，任务注明的进阶档） | ✅ | stdio 连接外部 MCP 服务器，启动时握手并发现工具。**注册插在 `WrapAllTools` 之前**，因此与内置工具走同一条 ACL 拦截（注入验证：挪到包装之后测试报 `ACL bypass`）；授权按角色授予，未授予的远端工具对所有人被拒（含 admin）；审批只认本地 `MCP_REQUIRE_APPROVAL`，不读远端 annotations；远端 schema 经 `ParamsOneOf` 保真；结果剥离协议信封只留文本；单个服务器连不上只记日志不阻止启动 |
...
| D6 | 单元测试 | ✅ | 53 个测试文件，292 个测试用例 |
| D7 | 集成测试 | ✅ | 13 个测试文件，42 个集成测试用例（含流式增量、工具进度、记忆开关、MCP 生命周期与 ACL 覆盖、前端 API 契约的双向断言） |

---

## 统计

- **硬性要求**：37 项 ✅ 通过，1 项 ⚠️ 部分达标
- **技术约束**：12 项 ✅ 全部满足
- **文档交付物**：7 项 ✅ 全部完成
...
线程历史默认无保留上限（已提供两个开关）、文件后端写入为全量重写、压缩后 checkpoint 体积上升、MCP 仅支持 stdio 传输。
...
**结论：项目核心功能完整。多用户隔离为框架强制（design.md 5.3 节）；节点级中断由请求显式标志触发；流式输出覆盖事件级进度与 token 级片段；压缩不再破坏完整历史；记忆开关在服务内部强制生效；按消息数裁剪已接入执行路径；向量召回受记忆预算约束；外部 MCP 工具与内置工具共用同一条 ACL 与审批链，无旁路。剩余一项部分达标（Checkpoint 恢复有降级路径）属合理设计。**
