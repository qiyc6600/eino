...
`integration_test/frontend_contract_test.go` 用真实路由驱动每个端点，断言前端读取的每个键确实存在。它还做反向检查：**该键必须真的出现在 `web/assets/app.js` 里**，否则测试失败——这样清单不会腐化成"断言一个没人用的契约"（第一次运行就抓出我列表里三个前端根本不读的键）。

覆盖两个方向：

| 方向 | 覆盖内容 |
|------|----------|
| **响应**（前端读什么） | 每个端点返回的键；SSE 的 `chunk`/`done` 帧；`done` 的 `status`/`answer`/`memory`/`events`/`contextTokens`/`actualTokens`；`tool_call` 帧的 `phase`/`tool`（**逐个帧**检查，不只第一个）；interrupt 对象的键 |
| **请求**（前端发什么） | 登录、聊天（含非流式回退的四个字段）、记忆写入、文档上传、切换模型、审批决策的载荷键名，并断言服务端接受它们 |

请求方向值得单独说：**改一个请求字段名会让处理器解出空字符串**——调用要么被拒，要么静默做错事，而所有响应断言仍然全绿。所以它比响应方向更容易悄悄坏掉。

SSE 部分另外断言服务端发出的帧名恰好是前端处理的那几个，以及 `tool_call` 的 `phase` 取值都在前端的 `switch` 分支里（新 phase 会静默落到默认分支）。所以"服务端不再发某个帧、前端留下死分支"和"服务端新增一个 phase、前端没有分支"都会被发现。
...
`Cache-Control: no-cache` 只保证**重载**能拿到新资源，它管不住一个已经打开的标签页——那个页面会继续执行它已经加载的 JS。所以服务端修好的 bug 在未刷新的标签页里看起来仍没修好：本项目就发生过一次（订单表格在修复后仍然溢出气泡，因为那个标签页是修复前加载的）。

机制：

| 环节 | 做法 |
|------|------|
| 版本号 | `web.AssetVersion()` 对嵌入资源算 sha256 内容哈希（按文件名排序）。用内容而不是构建时间——它要回答的是"文件变了吗"，时间戳会在内容相同的重建上报变化 |
| 页面知道自己是谁服务的 | 服务端把哈希注入 `index.html` 的 `<meta name="app-version">`，两个资产 URL 也带上 `?v=<哈希>` |
| 页面能问服务端在服务什么 | `/healthz` 返回 `assets` 字段。选它是因为它**无需认证且已存在**，不需要新路由，且会话过期后仍可用 |
| 不一致时 | 页面顶部出现"服务端已更新，当前页面仍在运行旧版本 [刷新]"。提示条在 `#mainApp` 之外，登录页也能看到——会话过期后又停留很久的标签页最容易撞上这种情况 |

两个资产的 `?v=` 参数过去是手工维护的（`?v=13`、`?v=26`），只有记得改才有效，而两者的数字本来就已经不一致了；现在都来自内容哈希。
...
> `weather` 是唯一真正发出网络请求的工具：调用 `wttr.in`（免费、无需 API key）。**上游不可用时它返回错误，不返回假数据**——早期版本会退化成一张硬编码的天气表（北京 32°C 晴…）并标成成功，调用方按成功/失败分支根本看不见这次降级，模型会把编造的温度当实测值报出来。判断降级是否正当只有一条标准：替代品是否仍然在说真话；而"此刻的真实天气"没有诚实的替代品，所以这里只能报错。
...
```
阈值 = 固定开销 + (MAX_TOKENS − 固定开销) × SUMMARIZE_THRESHOLD_RATIO
```

固定开销（工具 schema + 为回答预留）**直接加回、不参与比例缩放**——它无论有多少历史都要花。所以 token 条上的"阈值"背后是两个旋钮，管理员可以点条旁的 ⚙ 在运行时调整（窗口、触发比例、摘要目标），面板会显示当前值算出的阈值并说明钳制结果：

| 情形 | 行为 |
|------|------|
| 比例 > 1 | 钳到 1。否则触发点落在窗口之外，压缩**永不触发且无人察觉**（与 `MAX_MESSAGES` 设太小是同一个陷阱） |
| 窗口低于下限 | 钳到 256（`minUsableContextTokens`），**而不是**按填的存——报 10 而实际用 256 是撒谎 |
| 窗口小于固定开销 | 允许，但阈值以窗口封顶，界面直接说明"压缩会在任何历史出现时触发，请把窗口调大" |
| 摘要目标 | 只兜底到 1：摘要太短是看得见的，属于使用者的选择 |

被改动的字段名在 `PUT /api/context/settings` 的响应里回显（`clamped`），界面据此显示"已调整到可用范围"。**这些值不持久化**：重启后回到配置文件的值，面板上写明了这一点。`GET` 任何已登录用户可读（token 条要能解释自己），`PUT` 仅管理员——窗口是所有用户共享的。
...
| 请求体上限为 4MB（常量） | `maxRequestBytes` 包住整个 mux，超出返回 413；单个文档另有 200,000 字符上限。两者都是编译期常量，不通过环境变量配置 |
| 运行时调整的上下文设置不持久化 | 窗口与触发比例改了之后重启即回到配置文件的值，面板上写明了这一点 |
...
> **管理接口的角色校验**：`/api/users`（创建用户、改角色）与 `/api/models/switch` 通过 `requireAdmin` 要求 admin 角色（403）。这三处此前**只校验认证、不校验角色**——任何已登录账号（含 visitor）都能创建管理员或给自己提权。工具级 ACL 一直是强制执行的，这反而让缺口容易被忽略：框架保护的是"模型能调什么"，而发放角色的接口是敞开的。`GET /api/context/settings` 与 `/api/roles` 保持对所有已登录用户开放（前者让 token 条能解释自己，后者只是角色列表）。
